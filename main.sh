...
start_vm() {
    ...
    qemu-system-x86_64 -enable-kvm -m "$MEMORY" -smp "$CPUS" \
        ...
        $([ "$GUI_MODE" = "y" ] && echo "-vga virtio -display gtk,gl=on" || echo "-nographic")
}
...
[Service]
ExecStart=/usr/bin/screen -DmS zynex-vps bash -c '$HOME/ZynexVPS/main.sh'
Restart=always
RestartSec=5
User=$USER
